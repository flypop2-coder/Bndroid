//! Allocation-free M52 post-recovery Launcher-to-App focus-roundtrip evidence.
//!
//! M49, M50, and M51 remain immutable prefixes. This trace starts only after
//! the complete M51 snapshot is visible, then authenticates one App-only
//! contact, the resulting InputServer focus synchronization, one App
...
pub const APP_COLOR: u32 = 0x0084_4ec7;
...
pub struct PostRecoveryFocusRoundtripTraceSnapshot {
    pub bits: u64,
    pub errors: u64,
...
pub fn snapshot() -> PostRecoveryFocusRoundtripTraceSnapshot {
    ...
    PostRecoveryFocusRoundtripTraceSnapshot {
        bits,
        errors,
...
fn active_owners() -> Option<TraceOwners> {
    if !crate::post_recovery_focus_trace::snapshot().complete {
        return None;
    }
...
fn valid_bie(event: InputEvent, index: usize) -> bool {
    match (index, event.payload()) {
        (0, InputEventPayload::RoutedPointer(pointer)) => {
            event.sequence() == 15 && valid_pointer(pointer, 10, true)
        }
        (
            1,
        ...
        (2, InputEventPayload::RoutedPointer(pointer)) => {
            event.sequence() == 17 && valid_pointer(pointer, 11, false)
        }
        _ => false,
    }
}
...
fn valid_focus(command: InputCommand) -> bool {
    matches!(
        ...
        InputCommandPayload::SetFocus(Some(target))
            if command.sequence() == 4 && app_window_ref(target)
    )
}
...
fn valid_bwe(event: WindowEvent, index: usize) -> bool {
    match (index, event.payload()) {
        ...
        ) => event.command_sequence() == 6 && event.scene_frame() == 12 && app_window(window_id),
        _ => false,
    }
}
...
fn valid_present(command: WindowCommand) -> bool {
    matches!(
        ...
        WindowCommandPayload::Present {
            ...
            damage: APP_DAMAGE,
            color: APP_COLOR,
        } if command.sequence() == 6 && app_window(window_id)
    )
}

fn app_window_ref(target: WindowRef) -> bool {
...
mod tests {
    ...
    fn app_ref() -> WindowRef {
        WindowRef::try_new(InputWindowId::try_new(2).unwrap(), 1).unwrap()
    }

    fn app_window_id() -> WindowId {
    ...
    fn exact_transcript_is_fail_closed_and_then_ignores_resident_tail() {
        let owners = owners();
        let down = pointer_event(15, 10, true).encode();
        let focus = InputCommand::set_focus(4, Some(app_ref()))
            .unwrap()
            .encode();
        ...
        let ack = InputEvent::ack(16, CommandStream::Focus, 4)
            .unwrap()
            .encode();
        assert!(record_channel_write_with(
            owners,
        ...
        let up = pointer_event(17, 11, false).encode();
        assert!(record_channel_write_with(
            owners,
            INPUT_PID,
            UserImageId::InputServer,
        ...
        let present = WindowCommand::present(6, app_window_id(), 4, APP_DAMAGE, APP_COLOR)
            .unwrap()
            .encode();
        assert!(record_channel_write_with(
            owners,
        ...
        let presented = WindowEvent::presented(6, 12, app_window_id(), 4)
            .unwrap()
            .encode();
        assert!(record_channel_write_with(
            owners,
