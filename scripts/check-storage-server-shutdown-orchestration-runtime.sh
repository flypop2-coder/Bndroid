...
SHUTDOWN_PREFIX="STORAGE_SERVER_SHUTDOWN_ORCHESTRATION_OK"
BOOT_MARKER="BOOT_OK: M65 userspace StorageServer shutdown orchestration and durable close verified"
...
SHUTDOWN_PREFIX = "STORAGE_SERVER_SHUTDOWN_ORCHESTRATION_OK"
BOOT_MARKER = (
    "BOOT_OK: M65 userspace StorageServer shutdown orchestration and durable "
    "close verified"
)
...
SHUTDOWN_SCHEMA = [
    ("format", "1"),
    ("state_version", "1"),
    ("authority", "init-two-phase-shutdown"),
...
def validate_text(text: str, phase: str) -> None:
    reject(phase not in PHASES, "unknown M65 boot phase")
    lines = text.splitlines()
    reject(
        exact_marker(lines, HEALTH_PREFIX) != marker(HEALTH_PREFIX, HEALTH_SCHEMA, phase),
        "M65 health marker changed exact structure",
    )
    reject(
        exact_marker(lines, DATA_PREFIX) != data_marker(phase),
        "M65 DATA_PERSIST marker changed exact structure",
    )
    reject(
        exact_marker(lines, SHUTDOWN_PREFIX)
        != marker(SHUTDOWN_PREFIX, SHUTDOWN_SCHEMA, phase),
        "M65 shutdown marker changed exact structure",
    )
    reject(lines.count(BOOT_MARKER) != 1, "M65 BOOT marker changed")
    reject(
        any(line.startswith("BOOT_OK: M") and line != BOOT_MARKER for line in lines),
        "M65 emitted a stale BOOT marker",
    )
    reject(
        any(
        ...
        "M65 unexpectedly emitted a parent runtime marker",
    )
...
def expect_rejected(text: str, phase: str) -> None:
    try:
        ...
        return
    raise AssertionError("negative M65 parser fixture was accepted")
...
def self_test() -> None:
    ...
    for prefix, schema, index in (
        (HEALTH_PREFIX, HEALTH_SCHEMA, 0),
        (SHUTDOWN_PREFIX, SHUTDOWN_SCHEMA, 2),
    ):
    ...
    canonical_data = data_marker("first")
    for token in canonical_data.split()[1:]:
        name = token.split("=", 1)[0]
        mutated = canonical_data.replace(token, f"{name}=__bad__", 1)
        expect_rejected(first.replace(canonical_data, mutated, 1), "first")
        cases += 1
    ...
    cases += 2
    for stale in (
        "BOOT_OK: M64 stale parent",
        "STORAGE_SERVER_RUNTIME_OK stale=1",
        "STORAGE_SERVER_CLEAN_SHUTDOWN_OK stale=1",
    ):
    ...
    print(
        ...
        f"shutdown_fields={len(SHUTDOWN_SCHEMA)} phases=2"
    )
...
for tool in bash python3 cargo rustc qemu-system-aarch64 mktemp mkdir cp rm tr grep kill sleep; do
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "$tool not found; cannot verify M65 shutdown orchestration." >&2
    exit 1
  fi
done
if [[ "${BNDROID_PROFILE:-release}" != "release" ]]; then
  echo "M65 runtime evidence is release-only; BNDROID_PROFILE must be 'release'." >&2
  exit 2
fi
...
TARGET_ROOT="${BNDROID_STORAGE_SERVER_SHUTDOWN_TARGET_DIR:-$WORKSPACE_ROOT/target/storage-server-shutdown-orchestration-runtime-check}"
OUTPUT_DIR="$WORKSPACE_ROOT/target/m65"
mkdir -p "$TARGET_ROOT" "$OUTPUT_DIR"

TMP_DIR="$(mktemp -d "$WORKSPACE_ROOT/target/bndroid-storage-server-m65.XXXXXX")"
...
cleanup() {
  ...
  if [[ "$RUN_SUCCEEDED" == "1" && "$KEEP_TEMP" == "0" ]]; then
    ...
    echo "M65 diagnostic artifacts retained at $TMP_DIR" >&2
  fi
  exit "$exit_status"
}
trap 'cleanup "$?"' EXIT
...
reject_failure() {
  ...
  if grep -Eqi 'fatal exception:|kernel panic:|panic:|panicked at|boot error:|(^|[[:space:]_:])(failed|failure|diag|diagnostic)([[:space:]_:]|$)' "$log_file" \
    || grep -Eq '(^|[[:space:]_:])[A-Z0-9_]*FAIL(:|[[:space:]]|$)|M65_[A-Z0-9_]*TIMEOUT|^BOOT_OK: M(5[5-9]|6[0-4]) ' "$log_file"; then
    show_failure "$log_file"
    echo "M65 emitted a panic, failure, timeout, diagnostic, or stale BOOT marker." >&2
    exit 1
  fi
  if grep -Eq '^STORAGE_SERVER_(FAULT_POLICY|OWNER_LIVENESS|TERMINAL_QUARANTINE|CLEAN_SHUTDOWN|RUNTIME)_OK' "$log_file"; then
    show_failure "$log_file"
    echo "M65 emitted a destructive or pre-EL0 parent runtime marker." >&2
    exit 1
  fi
  if grep '^BOOT_OK:' "$log_file" | grep -Fvx "$BOOT_MARKER" >/dev/null; then
    show_failure "$log_file"
    echo "M65 published a stale or unrelated BOOT_OK marker." >&2
    exit 1
  fi
}
...
run_persistent_boot() {
  ...
  while ((SECONDS < deadline)); do
    ...
    shutdown_count="$(grep -Ec "^${SHUTDOWN_PREFIX}( |$)" "$normalized_log" || true)"
    boot_count="$(grep -Fxc "$BOOT_MARKER" "$normalized_log" || true)"
    if [[ "$health_count" == 1 && "$data_count" == 1 \
      && "$shutdown_count" == 1 && "$boot_count" == 1 ]]; then
      kill "$QEMU_PID" 2>/dev/null || true
      wait "$QEMU_PID" 2>/dev/null || true
      QEMU_PID=""
      tr -d '\r' <"$serial_log" >"$normalized_log"
    ...
    if ((health_count > 1 || data_count > 1 || shutdown_count > 1 || boot_count > 1)); then
      show_failure "$normalized_log"
      echo "M65 published duplicate health, persistence, shutdown, or BOOT evidence." >&2
      exit 1
    fi
    if ! kill -0 "$QEMU_PID" 2>/dev/null; then
      ...
      reject_failure "$normalized_log"
      show_failure "$normalized_log"
      echo "QEMU exited before M65 $phase-boot evidence (status $qemu_status)." >&2
      exit 1
    fi
    sleep 0.05
  done
  ...
  reject_failure "$normalized_log"
  show_failure "$normalized_log"
  echo "Timed out waiting for exact M65 $phase-boot evidence." >&2
  exit 1
}
...
if [[ ! -f "$KERNEL_IMAGE" ]]; then
  echo "M65 kernel image is missing: $KERNEL_IMAGE" >&2
  exit 1
fi
...
if len(before) != 8 * 1024 * 1024 or len(after) != len(before):
    raise SystemExit("M65 runtime disk lost its exact writable 8 MiB geometry")
...
if before[:data_start] != after[:data_start] or before[appdata_end:] != after[appdata_end:]:
    raise SystemExit("M65 QEMU changed bytes outside BNDROID_DATA/BNDROID_APPDATA")
if before[appdata_start:appdata_end] == after[appdata_start:appdata_end]:
    raise SystemExit("M65 did not persist its bounded AppData workload")
...
def decode_health_record(raw: bytes, expected_generation: int, boot_open: bool) -> None:
    if raw[:8] != fixture.DATA_RECORD_MAGIC:
        raise SystemExit("M65 record magic changed")
    if struct.unpack_from("<I", raw, 8)[0] != fixture.DATA_FORMAT_VERSION:
        raise SystemExit("M65 record version changed")
    if struct.unpack_from("<I", raw, 12)[0] != fixture.DATA_RECORD_HEADER_BYTES:
        raise SystemExit("M65 record header size changed")
...
if after[unused_start:data_end] != before[unused_start:data_end] or any(
    ...
    raise SystemExit("M65 changed an unused BNDROID_DATA sector")
...
print(
    ...
    "userspace_shutdowns=2 clients_drained=4 storage_server_flushes=2 "
    "storage_server_readbacks=2 storage_server_exits=2 prepare_calls=8 "
    "prepares=2 commit_calls=2 commits=2 spawn_rejections=2 "
    "final_appdata_generation=6 final_health_generation=4 final_health_slot=0 "
    "prior_health_generation=3 prior_health_slot=1 final_boot_open=0 "
    "prior_boot_open=1 qemu_backend_persistence=1 "
    ...
    "offline_persisted=0 offline_from_record=0 el0_started=1 el0_controls=1 "
...
{
  printf '%s\n' "M65_BOOT_SEQUENCE phase=first"
  cat "$FIRST_LOG"
  printf '%s\n' "M65_BOOT_SEQUENCE phase=second"
  cat "$SECOND_LOG"
  printf '%s\n' "$REBOOT_MARKER"
} >"$OUTPUT_DIR/storage-server-shutdown-orchestration-runtime.log"

RUN_SUCCEEDED=1
echo "M65 userspace shutdown-orchestration reboot self-test passed."
