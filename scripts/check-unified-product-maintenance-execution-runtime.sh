...
for tool in bash python3 cargo rustc qemu-system-aarch64 mktemp mkdir cp rm tee grep tr sed; do
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "$tool not found; cannot verify M78 maintenance execution runtime." >&2
    exit 1
  fi
done

if [[ "${BNDROID_PROFILE:-release}" != "release" ]]; then
  echo "M78 runtime evidence is release-only; BNDROID_PROFILE must be 'release'." >&2
  exit 2
fi
...
SEED_TARGET_ROOT="${BNDROID_M78_SEED_TARGET_DIR:-$WORKSPACE_ROOT/target/m78-key-rotation-seed-check}"
TRANSITION_TARGET_ROOT="${BNDROID_M78_TRANSITION_TARGET_DIR:-$WORKSPACE_ROOT/target/m78-key-rotation-transition-check}"
ACTIVE_TARGET_ROOT="${BNDROID_M78_ACTIVE_TARGET_DIR:-$WORKSPACE_ROOT/target/m78-key-rotation-active-check}"
SEQUENCE1_TARGET_ROOT="${BNDROID_M78_SEQUENCE1_TARGET_DIR:-$WORKSPACE_ROOT/target/maintenance-execution-sequence1-check}"
SEQUENCE2_TARGET_ROOT="${BNDROID_M78_SEQUENCE2_TARGET_DIR:-$WORKSPACE_ROOT/target/maintenance-execution-sequence2-check}"
SIGNATURE_TARGET_ROOT="${BNDROID_M78_BAD_SIGNATURE_TARGET_DIR:-$WORKSPACE_ROOT/target/maintenance-execution-bad-signature-check}"
BINDING_TARGET_ROOT="${BNDROID_M78_WRONG_BINDING_TARGET_DIR:-$WORKSPACE_ROOT/target/maintenance-execution-wrong-binding-check}"
OUTPUT_DIR="$WORKSPACE_ROOT/target/m78"
...
mkdir -p \
  ...
  "$BINDING_TARGET_ROOT" \
  "$OUTPUT_DIR"
TMP_DIR="$(mktemp -d "$WORKSPACE_ROOT/target/bndroid-unified-product-m78.XXXXXX")"
RUNTIME_IMAGE="$TMP_DIR/runtime.raw"
BASE_IMAGE="$TMP_DIR/base-key4.raw"
SEQUENCE1_IMAGE="$TMP_DIR/sequence1-complete.raw"
INTERRUPT_IMAGE="$TMP_DIR/sequence2-interrupted.raw"
RESUME_IMAGE="$TMP_DIR/sequence2-resumed.raw"
SIGNATURE_IMAGE="$TMP_DIR/signature.raw"
BINDING_IMAGE="$TMP_DIR/binding.raw"
REPLAY_IMAGE="$TMP_DIR/completed-replay.raw"
...
INTERRUPT_SERIAL="$TMP_DIR/interrupt.serial.log"
RESUME_SERIAL="$TMP_DIR/resume.serial.log"
...
INTERRUPT_DRIVER="$TMP_DIR/interrupt.driver.log"
RESUME_DRIVER="$TMP_DIR/resume.driver.log"
...
INTERRUPT_QMP="$TMP_DIR/interrupt.qmp"
RESUME_QMP="$TMP_DIR/resume.qmp"
SIGNATURE_QMP="$TMP_DIR/signature.qmp"
BINDING_QMP="$TMP_DIR/binding.qmp"
REPLAY_QMP="$TMP_DIR/completed-replay.qmp"
...
RESUME_SCREENSHOT="$TMP_DIR/resume.ppm"
RUN_SUCCEEDED=0
...
cleanup() {
  ...
  if [[ "$RUN_SUCCEEDED" == "1" && "$KEEP_TEMP" == "0" ]]; then
    ...
    echo "M78 diagnostic artifacts retained at $TMP_DIR" >&2
  fi
  exit "$exit_status"
}
trap 'cleanup "$?"' EXIT
...
build_kernel "$SEQUENCE2_TARGET_ROOT" "$M78_FEATURE" "$ACTIVE_MANIFEST" "$SEQUENCE2_ARTIFACT"
build_kernel "$SIGNATURE_TARGET_ROOT" "$M78_FEATURE" "$ACTIVE_MANIFEST" "$BAD_SIGNATURE_ARTIFACT"
build_kernel "$BINDING_TARGET_ROOT" "$M78_FEATURE" "$ACTIVE_MANIFEST" "$WRONG_BINDING_ARTIFACT"
...
for image in \
  ...
  "$BINDING_KERNEL"; do
  if [[ ! -f "$image" ]]; then
    echo "M78 kernel image is missing: $image" >&2
    exit 1
  fi
done
...
run_positive() {
  ...
  if [[ "$(grep -Fxc "$expected_driver" "$driver" || true)" != 1 ]]; then
    echo "M78 $phase boot driver evidence changed." >&2
    exit 1
  fi
  case "$milestone" in
    M75)
    ...
    M78)
      ...
      ;;
    *)
      echo "unexpected M78 staged milestone: $milestone" >&2
      exit 1
      ;;
  esac
}
...
run_interrupted_sequence2() {
  python3 "$SCRIPT_DIR/maintenance_execution_interrupt_qemu.py" \
    --kernel "$SEQUENCE2_KERNEL" \
    --disk "$RUNTIME_IMAGE" \
    --serial "$INTERRUPT_SERIAL" \
    --qmp "$INTERRUPT_QMP" \
    --sequence 2 \
    --cpu "$QEMU_CPU" \
    --timeout "$BOOT_TIMEOUT_SECONDS" \
    | tee "$INTERRUPT_DRIVER"
  python3 "$SCRIPT_DIR/unified_product_maintenance_execution_evidence.py" \
    parse-interrupt "$INTERRUPT_SERIAL" "$INTERRUPT_DRIVER"
}
...
run_negative() {
  ...
  expected_driver+="audit_slots_mutated=0$execution_evidence "
  ...
  python3 "$SCRIPT_DIR/maintenance_authorization_negative_qemu.py" \
    ...
    | tee "$driver"
  if [[ "$(grep -Fxc "$expected_driver" "$driver" || true)" != 1 ]]; then
    echo "M78 $reason negative host evidence changed." >&2
    exit 1
  fi
  python3 "$SCRIPT_DIR/unified_product_maintenance_execution_evidence.py" \
...
run_positive sequence1 M78 "$SEQUENCE1_KERNEL" \
  "$SEQUENCE1_SERIAL" "$SEQUENCE1_DRIVER" "$SEQUENCE1_QMP" "$SEQUENCE1_SCREENSHOT"
cp "$RUNTIME_IMAGE" "$SEQUENCE1_IMAGE"

run_interrupted_sequence2
cp "$RUNTIME_IMAGE" "$INTERRUPT_IMAGE"

run_positive resume M78 "$SEQUENCE2_KERNEL" \
  "$RESUME_SERIAL" "$RESUME_DRIVER" "$RESUME_QMP" "$RESUME_SCREENSHOT"
cp "$RUNTIME_IMAGE" "$RESUME_IMAGE"
...
REBOOT_MARKER="$(
  python3 "$SCRIPT_DIR/unified_product_maintenance_execution_evidence.py" \
    reboot \
    "$BASE_IMAGE" "$SEQUENCE1_IMAGE" "$INTERRUPT_IMAGE" "$RESUME_IMAGE" \
    "$SIGNATURE_IMAGE" "$BINDING_IMAGE" "$REPLAY_IMAGE" \
    "$SEQUENCE1_SERIAL" "$SEQUENCE1_DRIVER" \
    "$INTERRUPT_SERIAL" "$INTERRUPT_DRIVER" \
    "$RESUME_SERIAL" "$RESUME_DRIVER" \
...

{
  ...
  for phase in seed-m75 transition-m76 activate-m76 repair-m76; do
    printf '%s\n' "M78_PREREQUISITE_BOOT_SEQUENCE phase=$phase qemu_psci_self_exit=1"
    case "$phase" in
      seed-m75)
        tr -d '\r' <"$SEED_SERIAL"
        sed '/^$/d' "$SEED_DRIVER"
  ...
  printf '%s\n' "M78_BOOT_SEQUENCE phase=sequence1 qemu_psci_self_exit=1"
  tr -d '\r' <"$SEQUENCE1_SERIAL"
  sed '/^$/d' "$SEQUENCE1_DRIVER"
  printf '%s\n' "M78_INTERRUPT_BOOT_SEQUENCE phase=sequence2 host_terminated=1 state=audit2/execution1"
  tr -d '\r' <"$INTERRUPT_SERIAL"
  sed '/^$/d' "$INTERRUPT_DRIVER"
  printf '%s\n' "M78_BOOT_SEQUENCE phase=resume qemu_psci_self_exit=1 state=audit2/execution2"
  tr -d '\r' <"$RESUME_SERIAL"
  sed '/^$/d' "$RESUME_DRIVER"
  for reason in signature binding completed-replay; do
    printf '%s\n' "M78_NEGATIVE_BOOT_SEQUENCE reason=$reason host_terminated=1"
    case "$reason" in
      signature)
  ...
  printf '%s\n' "$REBOOT_MARKER"
} >"$OUTPUT_DIR/unified-product-maintenance-execution-runtime.log"
cp "$SEQUENCE1_SCREENSHOT" "$OUTPUT_DIR/sequence1.ppm"
cp "$RESUME_SCREENSHOT" "$OUTPUT_DIR/resume.ppm"

RUN_SUCCEEDED=1
echo "M78 durable maintenance execution and interrupted-session recovery passed."
