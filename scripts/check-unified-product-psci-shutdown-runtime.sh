...
if [[ "${1:-}" == "--parse" && "$#" == 3 ]]; then
  python3 "$SCRIPT_DIR/unified_product_psci_shutdown_evidence.py" \
    parse "$2" "$3"
  exit 0
fi
if [[ "$#" -ne 0 ]]; then
...
for tool in bash python3 cargo rustc qemu-system-aarch64 mktemp mkdir cp rm tee grep tr sed; do
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "$tool not found; cannot verify M70 QEMU PSCI shutdown runtime." >&2
    exit 1
  fi
done

if [[ "${BNDROID_PROFILE:-release}" != "release" ]]; then
  echo "M70 runtime evidence is release-only; BNDROID_PROFILE must be 'release'." >&2
  exit 2
fi
...
TARGET_ROOT="${BNDROID_UNIFIED_PRODUCT_PSCI_SHUTDOWN_TARGET_DIR:-$WORKSPACE_ROOT/target/unified-product-psci-shutdown-runtime-check}"
OUTPUT_DIR="$WORKSPACE_ROOT/target/m70"
...
TMP_DIR="$(mktemp -d "$WORKSPACE_ROOT/target/bndroid-unified-product-m70.XXXXXX")"
...
cleanup() {
  ...
  if [[ "$RUN_SUCCEEDED" == "1" && "$KEEP_TEMP" == "0" ]]; then
    ...
    echo "M70 diagnostic artifacts retained at $TMP_DIR" >&2
  fi
  exit "$exit_status"
}
trap 'cleanup "$?"' EXIT
...
if [[ ! -f "$KERNEL_IMAGE" ]]; then
  echo "M70 kernel image is missing: $KERNEL_IMAGE" >&2
  exit 1
fi
...
run_boot() {
  ...
  if [[ "$(grep -Fxc "$expected_driver" "$driver" || true)" != 1 ]]; then
    echo "M70 $phase boot driver evidence changed." >&2
    exit 1
  fi
  python3 "$SCRIPT_DIR/unified_product_psci_shutdown_evidence.py" \
    parse "$phase" "$serial"
...
REBOOT_MARKER="$(
  python3 "$SCRIPT_DIR/unified_product_psci_shutdown_evidence.py" reboot \
...
{
  printf '%s\n' "M70_BOOT_SEQUENCE phase=first qemu_psci_self_exit=1"
  tr -d '\r' <"$FIRST_SERIAL"
  sed '/^$/d' "$FIRST_DRIVER"
  printf '%s\n' "M70_BOOT_SEQUENCE phase=second qemu_psci_self_exit=1"
  tr -d '\r' <"$SECOND_SERIAL"
  sed '/^$/d' "$SECOND_DRIVER"
  printf '%s\n' "$REBOOT_MARKER"
} >"$OUTPUT_DIR/unified-product-psci-shutdown-runtime.log"

RUN_SUCCEEDED=1
echo "M70 two-boot FDT-validated QEMU PSCI/UI/AppData/shutdown self-exit test passed."
