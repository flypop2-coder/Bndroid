#!/usr/bin/env python3
"""Boot one M76 artifact until its pre-EL0 key-policy rejection."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reason", choices=("signature", "retired-key"), required=True
    )
...
def wait_for_rejection(
    ...
    marker = f"KEY_ROTATION_REJECTED format=1 reason={reason} "
    diagnostic = "boot error: manifest key-rotation preparation failed:"
    ...
    while time.monotonic() < deadline:
        ...
        if unexpected:
            raise product.RuntimeFailure(
                f"unexpected guest failure during M76 negative boot: {unexpected[-1]}"
            )
...
def main() -> int:
    ...
    try:
        ...
        for marker in forbidden:
            if marker in log:
                raise product.RuntimeFailure(
                    f"M76 negative boot crossed fail-closed boundary: {marker}"
                )
        rejection = [
            line
            for line in log.splitlines()
            if line.startswith(
                f"KEY_ROTATION_REJECTED format=1 reason={args.reason} "
            )
        ]
        if len(rejection) != 1:
            raise product.RuntimeFailure(
        ...
        signature_valid = int(args.reason == "retired-key")
        print(
            "KEY_ROTATION_NEGATIVE_BOOT_OK "
            f"reason={args.reason} pre_el0=1 signature_valid={signature_valid} "
            "manifest_published=0 init_ready=0 policy_slots_mutated=0 "
