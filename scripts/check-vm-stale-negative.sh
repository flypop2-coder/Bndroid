...
TARGET_ROOT="$WORKSPACE_ROOT/target/vm-bookkeeping-only-unmap-self-test"
CARGO_TARGET_DIR="$TARGET_ROOT" cargo build \
  --locked \
  --target aarch64-unknown-none \
  -p bndroid-kernel \
  --features vm-bookkeeping-only-unmap-self-test
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/bndroid-vm-stale.XXXXXX")"
...
while ((SECONDS < deadline)); do
  ...
  if grep -Eq '^FRAME_OK |^VM_OK |^MEMORY_OK:|^HEAP_OK |^HEAP_IRQ_OK |^SCHED_OK |^SLEEP_OK |^BOOT_OK:' "$NORMALIZED_LOG"; then
    cat "$NORMALIZED_LOG"
    echo "Bookkeeping-only unmap injection reached a success marker unexpectedly." >&2
    exit 1
  fi
  if grep -Fq 'VM_STALE_MAPPING_INJECTED: bookkeeping-only unmap left old sentinel' "$NORMALIZED_LOG" \
    && grep -Fq 'kernel panic:' "$NORMALIZED_LOG" \
    && grep -Fq 'dynamic unmap changed bookkeeping without clearing the hardware PTE' "$NORMALIZED_LOG"; then
    if ! grep -Fq 'entered AArch64 EL1; running EL1' "$NORMALIZED_LOG"; then
      cat "$NORMALIZED_LOG"
      echo "VM stale-mapping test did not run directly at EL1." >&2
      exit 1
    fi
    cat "$NORMALIZED_LOG"
    echo "VM stale-mapping self-test passed: bookkeeping-only unmap was rejected."
    exit 0
  fi
  if ! kill -0 "$QEMU_PID" 2>/dev/null; then
    ...
    QEMU_PID=""
    cat "$NORMALIZED_LOG"
    echo "QEMU exited before the expected stale-mapping failure (status $status)." >&2
    exit 1
  fi
  sleep 0.1
done
...
tr -d '\r' <"$LOG_FILE"
echo "Timed out waiting for the expected stale-mapping failure." >&2
exit 1
