...
for tool in qemu-system-aarch64 python3 mktemp tr grep tail kill cp mkdir rm sleep cat; do
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "$tool not found; cannot verify the M52 post-recovery focus roundtrip runtime." >&2
    exit 1
  fi
done
...
DISCOVER_HASHES="${BNDROID_POST_RECOVERY_FOCUS_ROUNDTRIP_DISCOVER_HASHES:-0}"
...
if [[ "$DISCOVER_HASHES" != "0" && "$DISCOVER_HASHES" != "1" ]]; then
  echo "BNDROID_POST_RECOVERY_FOCUS_ROUNDTRIP_DISCOVER_HASHES must be 0 or 1." >&2
  exit 2
fi

# These two immutable prefix hashes remain mandatory even during discovery.
# Only the final M52 hash may start as the all-zero controlled placeholder.
...
for expected in "$EXPECTED_M50_SHA256" "$EXPECTED_M51_SHA256" "$EXPECTED_M52_SHA256"; do
  if [[ ! "$expected" =~ ^[0-9a-f]{64}$ ]]; then
    echo "M52 screenshot SHA-256 constants must be canonical lowercase hexadecimal." >&2
    exit 2
  fi
done
if [[ "$EXPECTED_M50_SHA256" == "$ZERO_SHA256" || "$EXPECTED_M51_SHA256" == "$ZERO_SHA256" ]]; then
  echo "M52 discovery cannot weaken the frozen M50 or M51 screenshot hashes." >&2
  exit 2
fi
if [[ "$DISCOVER_HASHES" == "0" && "$EXPECTED_M52_SHA256" == "$ZERO_SHA256" ]]; then
  echo "M52 final screenshot SHA-256 is not frozen; run once with BNDROID_POST_RECOVERY_FOCUS_ROUNDTRIP_DISCOVER_HASHES=1, then replace only EXPECTED_M52_SHA256." >&2
  exit 2
fi
...
TARGET_ROOT="${BNDROID_POST_RECOVERY_FOCUS_ROUNDTRIP_TARGET_DIR:-$WORKSPACE_ROOT/target/post-recovery-focus-roundtrip-runtime}"
CARGO_TARGET_DIR="$TARGET_ROOT" \
  BNDROID_PROFILE="$PROFILE" \
  BNDROID_USERSPACE_FEATURES=post-recovery-focus-roundtrip-runtime \
  BNDROID_KERNEL_FEATURES=post-recovery-focus-roundtrip-runtime,surface-trace-evidence \
  "$SCRIPT_DIR/build-kernel.sh"

KERNEL_IMAGE="$TARGET_ROOT/aarch64-unknown-none/$PROFILE/bndroid-kernel.img"
if [[ ! -f "$KERNEL_IMAGE" ]]; then
  echo "M52 post-recovery focus roundtrip kernel image is missing: $KERNEL_IMAGE" >&2
  exit 1
fi

TMP_DIR="$(mktemp -d "$WORKSPACE_ROOT/target/bndroid-post-recovery-m52.XXXXXX")"
...
OUTPUT_DIR="$WORKSPACE_ROOT/target/m52"
QEMU_PID=""
: >"$SERIAL_LOG"
: >"$QEMU_LOG"
...
BOOT_MARKER="BOOT_OK: M52 post-recovery Launcher-to-App focus roundtrip and frame verified"
...
cleanup() {
  ...
  if [[ "$KEEP_TEMP" == "1" ]]; then
    echo "M52 diagnostic artifacts retained at $TMP_DIR" >&2
  else
    rm -rf "$TMP_DIR"
  fi
}
...
reject_failure() {
  ...
  if grep -Eqi 'fatal exception:|kernel panic:|panic:|panicked at|boot error:|BOOT_FAIL|USER_FAIL:|USER_FAIL |EL0_FAIL|THREAD_EXITED:|post-recovery (interaction|focus|focus roundtrip) runtime (failed|timed out)|service[- ]dependency runtime (failed|timed out)|^[A-Z0-9_]+_(DIAG|TIMEOUT|FAIL|FAILED)(:| |$)' "$NORMALIZED_LOG"; then
    show_failure
    echo "M52 emitted a panic, failure, diagnostic, timeout, or failed marker." >&2
    exit 1
  fi
...
validate_screenshots() {
  python3 - \
    ...
    "$EXPECTED_M50_SHA256" \
    "$EXPECTED_M51_SHA256" \
    "$EXPECTED_M52_SHA256" <<'PY'
import hashlib
import re
import sys

m50_path, m51_path, m52_path = sys.argv[1:4]
discover = sys.argv[4] == "1"
expected_m50, expected_m51, expected_m52 = sys.argv[5:8]
...
if not discover:
    if not re.fullmatch(r"[0-9a-f]{64}", expected_m52) or expected_m52 == "0" * 64:
        raise SystemExit("M52 final expected SHA-256 is not frozen")
    if m52_hash != expected_m52:
        raise SystemExit(f"M52 final SHA-256 changed: {m52_hash}, expected {expected_m52}")
print(m50_hash, m51_hash, m52_hash)
PY
}
...
boot = "BOOT_OK: M52 post-recovery Launcher-to-App focus roundtrip and frame verified"
if lines.count(boot) != 1:
    raise SystemExit("M52 boot marker is absent or duplicated")
...
if positions != sorted(positions) or len(set(positions)) != len(positions):
    raise SystemExit(f"M49/M50/M51/M52 proof markers are out of order: {positions!r}")
...
for line in lines:
    if (
        ...
        raise SystemExit("retired SurfaceServer input ownership leaked into M52")
PY
}
...
# M52 alone owns the terminal BOOT_OK. Its App down is physical 10 and its
# release is physical 11; the final screenshot follows the committed frame 6.
...
wait_for_prefix_once "$ROUNDTRIP_SUCCESS" "the final M52 roundtrip marker"
wait_for_exact_once "$BOOT_MARKER" "the final M52 boot marker"

tr -d '\r' <"$SERIAL_LOG" >"$NORMALIZED_LOG"
if ! reject_failure; then
  show_failure
  echo "M52 serial log ended with an incomplete evidence line." >&2
  exit 1
fi
validate_final_evidence
if ! validate_storage_success_evidence "$NORMALIZED_LOG"; then
  show_failure
  echo "M52 did not preserve the exact unique M25 storage evidence." >&2
  exit 1
fi
if [[ "$(grep -Fc 'entered AArch64 EL1' "$NORMALIZED_LOG" || true)" != "1" \
  || "$(grep -Fc 'running EL1' "$NORMALIZED_LOG" || true)" != "1" ]]; then
  show_failure
  echo "M52 did not publish unique entered/running EL1 evidence." >&2
  exit 1
fi

read -r M50_SHA256 M51_SHA256 M52_SHA256 < <(validate_screenshots)
...
if [[ -e "$QUIT_TIMEOUT_FLAG" ]]; then
  show_failure
  echo "QEMU did not exit within ${BOOT_TIMEOUT_SECONDS}s after the M52 QMP quit." >&2
  exit 1
fi
if [[ "$QEMU_STATUS" != "0" ]]; then
  show_failure
  echo "QEMU did not exit cleanly after the M52 QMP quit (status $QEMU_STATUS)." >&2
  exit 1
fi
...
if ! reject_failure; then
  show_failure
  echo "M52 serial log ended with an incomplete evidence line after QMP quit." >&2
  exit 1
fi
validate_final_evidence
if ! validate_storage_success_evidence "$NORMALIZED_LOG"; then
  show_failure
  echo "M52 post-quit storage evidence changed or duplicated." >&2
  exit 1
fi

if [[ "$DISCOVER_HASHES" == "1" ]]; then
  echo "POST_RECOVERY_FOCUS_ROUNDTRIP_DISCOVER_HASHES m50_sha256=$M50_SHA256 m51_sha256=$M51_SHA256 m52_sha256=$M52_SHA256 diffs=1280/1280 damage=64/80/40/32+160/216/40/32"
else
  echo "POST_RECOVERY_FOCUS_ROUNDTRIP_QMP_OK pointer=136/184/down-captured-up target=app focus=launcher4-app5 output=5-6 diffs=1280/1280 damage=64/80/40/32+160/216/40/32 m50_sha256=$M50_SHA256 m51_sha256=$M51_SHA256 m52_sha256=$M52_SHA256 markers=m49-prefix/m50-prefix/m51-prefix/roundtrip-armed/app-captured/app-presented/roundtrip/boot"
fi
