...
# Strict offline ABI-51 AndroidApp APK Envelope-4 gate.
#
# This remains a bounded compatibility proof. It builds the mixed-method APK
# twice with the Mac's installed SDK, verifies its real ZIP envelope, performs
# one sourced install boot, then one source-free recovery boot. The recovery
# boot proves the five-node publisher scene, one controlled worker replacement,
# two independently addressed callback branches, and an ordinary source-free
# relaunch through the same replacement worker.
#
...
if (($#)); then
  printf '%s\n' 'Usage: check-androidbox-envelope4.sh' >&2
  exit 2
fi

for tool in qemu-system-aarch64 python3 mktemp tr grep kill tail mkdir sleep \
  awk cmp cp shasum wc sed head sort unzip; do
  command -v "$tool" >/dev/null 2>&1 || {
    echo "$tool not found; cannot run the APK Envelope-4 gate." >&2
    exit 1
  }
done
...
FIXTURE_SHA256="$FIXTURE_DIR/androidbox-envelope-demo.apk.sha256"
...
[[ -f "$FIXTURE_SHA256" ]] || {
  echo "Required fixture digest sidecar is missing: $FIXTURE_SHA256" >&2
  exit 1
}
...
for sdk_tool in "$AAPT2" "$APKSIGNER" "$DEXDUMP"; do
  [[ "$sdk_tool" == /* && -x "$sdk_tool" ]] || {
...
mkdir -p "$WORKSPACE_ROOT/target/androidbox-envelope4"
ARTIFACT_DIR="$(mktemp -d "$WORKSPACE_ROOT/target/androidbox-envelope4/check.XXXXXX")"
TMPDIR="$ARTIFACT_DIR/tmp"
mkdir -p "$TMPDIR"
export TMPDIR

SOURCE_APK="$ARTIFACT_DIR/envelope.apk"
FIRST_APK="$ARTIFACT_DIR/envelope.first.apk"
...
ENVELOPE_EVIDENCE="$ARTIFACT_DIR/apk-envelope-evidence.txt"
RESOURCES_LOG="$ARTIFACT_DIR/resources.txt"
MANIFEST_XML_LOG="$ARTIFACT_DIR/manifest-xmltree.txt"
...
TARGET_ROOT="$WORKSPACE_ROOT/target/androidbox-envelope4-build"
...
start_qemu() {
  local mode="$1"
  local source_apk="$2"
  local -a source_args=(-name "Bndroid APK Envelope-4 Gate $mode")
...
# Build the real mixed-method APK twice with local SDK tools. These are two
# complete compile/link/DEX/align/v2-sign passes, not two signatures over one
# already-built unsigned archive.
...
[[ "$APK_BYTES" == "$EXPECTED_APK_BYTES" && "$APK_SHA256" == "$EXPECTED_APK_SHA256" ]] \
  || { echo "Pinned APK Envelope-4 length or digest changed." >&2; exit 1; }
((APK_BYTES > 0 && APK_BYTES <= 65024)) \
  || { echo "APK exceeds the bounded package size." >&2; exit 1; }
read -r SIDECAR_SHA256 SIDECAR_NAME <"$FIXTURE_SHA256"
[[ "$SIDECAR_SHA256" == "$APK_SHA256" && "$SIDECAR_NAME" == "androidbox-envelope-demo.apk" ]] \
  || { echo "Fixture SHA-256 sidecar is not canonical." >&2; exit 1; }
...
grep -Fqx "launchable-activity: name='org.bndroid.envelope.MainActivity'  label='' icon=''" "$BADGING_LOG" \
  || { echo "aapt2 launcher component mismatch." >&2; exit 1; }
grep -Fq 'Envelope review' "$RESOURCES_LOG" \
  || { echo "aapt2 title resource mismatch." >&2; exit 1; }
[[ "$(grep -Fc 'E: uses-permission' "$MANIFEST_XML_LOG" || true)" == 0 ]] \
  || { echo "Envelope fixture unexpectedly requests an Android permission." >&2; exit 1; }
grep -Fqx "Signer #1 certificate SHA-256 digest: $EXPECTED_SIGNER_SHA256" "$SIGNER_LOG" \
  || { echo "APK signer mismatch." >&2; exit 1; }
for disabled_scheme in \
  'Verified using v1 scheme (JAR signing): false' \
  'Verified using v3 scheme (APK Signature Scheme v3): false' \
  'Verified using v4 scheme (APK Signature Scheme v4): false'; do
  grep -Fqx "$disabled_scheme" "$SIGNER_LOG" \
    || { echo "APK enabled an unrequested signature scheme." >&2; exit 1; }
done
grep -Fqx 'Verified using v2 scheme (APK Signature Scheme v2): true' "$SIGNER_LOG" \
  || { echo "APK v2 signature was not verified." >&2; exit 1; }

python3 - "$SOURCE_APK" "$ENVELOPE_EVIDENCE" <<'PY'
from pathlib import Path
import struct
import sys
import zipfile

apk_path, evidence_path = map(Path, sys.argv[1:])
...
with zipfile.ZipFile(apk_path) as archive:
    ...
    if [entry.filename for entry in entries] != names:
        raise SystemExit("APK Envelope-4 entry order is not exact")
    if [entry.compress_type for entry in entries] != methods:
        raise SystemExit("APK Envelope-4 methods are not 8/8/0/0/8")
    if sum(entry.filename == "classes.dex" for entry in entries) != 1:
        raise SystemExit("APK must contain exactly one classes.dex")
    if archive.testzip() is not None:
        raise SystemExit("APK Envelope-4 CRC validation failed")
    descriptors = []
    for entry in entries:
        if entry.flag_bits & 1:
            raise SystemExit(f"encrypted APK entry: {entry.filename}")
        ...
        if signature != 0x04034B50:
            raise SystemExit(f"bad local header: {entry.filename}")
        if local_flags != entry.flag_bits or local_method != entry.compress_type:
            raise SystemExit(f"local/central mismatch: {entry.filename}")
        if (
            local_crc != entry.CRC
            ...
            raise SystemExit(
                f"local tuple is not central-exact: {entry.filename}"
            )
        data_offset = offset + 30 + name_length + extra_length
        if entry.filename in ("resources.arsc", "classes.dex") \
                and data_offset % 4:
            raise SystemExit(f"unaligned STORED entry: {entry.filename}")
        if entry.flag_bits & 8:
            marker, crc32, compressed, uncompressed = struct.unpack_from(
                "<IIII", raw, data_offset + entry.compress_size
            )
            if (
                marker != 0x08074B50
                or crc32 != entry.CRC
                ...
                raise SystemExit(f"bad data descriptor: {entry.filename}")
            descriptors.append(entry.filename)
    if not descriptors:
        raise SystemExit("APK has no bit-3 data descriptor")
    if "assets/envelope-note.txt" not in descriptors:
        raise SystemExit("DEFLATE envelope asset lacks a data descriptor")
    if not archive.read("assets/envelope-note.txt").startswith(
        b"AndroidBox APK Envelope-4 fixture note."
    ):
        raise SystemExit("uninterpreted envelope asset payload changed")
    dex = archive.read("classes.dex")
    if not dex.startswith((b"dex\n035\0", b"dex\n037\0", b"dex\n038\0", b"dex\n039\0")):
        raise SystemExit("classes.dex magic is unsupported")
    Path(sys.argv[1]).with_name("classes.dex").write_bytes(dex)

evidence_path.write_text(
    "\n".join((
        "entry_order=" + ",".join(names),
        "entry_methods=8,8,0,0,8",
        "descriptor_entries=" + ",".join(descriptors),
...
[[ "$(grep -Fxc "  Class descriptor  : '$ACTIVITY'" "$DEX_LOG" || true)" == 1 ]] \
  || { echo "DEX does not contain exactly one manifest Activity descriptor." >&2; exit 1; }
for method in \
  "name          : 'onCreate'" \
  "name          : 'onClick'" \
  "setContentView:(I)V" \
  ...
  "getId:()I" \
  "setText:(I)V"; do
  grep -Fq "$method" "$DEX_LOG" \
    || { echo "DEX method shape is missing: $method" >&2; exit 1; }
done

ENVELOPE4_FEATURES=mobile-ui-runtime,androidbox-dex0,androidbox-apk-install0,androidbox-el0-runtime0,androidbox-interactive0,androidbox-process0,androidbox-restart0,androidbox-scene-rpc2,androidbox-multiaction3,androidbox-apk-envelope4
if ! CARGO_TARGET_DIR="$TARGET_ROOT" \
  BNDROID_PROFILE=release \
  BNDROID_KERNEL_FEATURES="$ENVELOPE4_FEATURES" \
  BNDROID_USERSPACE_FEATURES="$ENVELOPE4_FEATURES" \
  "$BUILD_KERNEL" >"$BUILD_LOG" 2>&1; then
  tail -n 320 "$BUILD_LOG" >&2
  echo "ABI-51 APK Envelope-4 build failed." >&2
  exit 1
fi
...
wait_for_pattern \
  ...
  "the inherited ABI-51 MultiActionActivity-3 profile marker"
wait_for_pattern \
  "^ANDROIDBOX_APK_ENVELOPE4_PROFILE_OK format=1 abi=${ABI} parent_profile=androidbox-multiaction3 package=${PACKAGE} apk_source=durable-package-store-readback manifest_method=deflate layout_method=deflate classes_dex_method=stored resources_arsc_method=stored data_descriptors=3 incidental_deflated_entries=1 bit3_local_tuple_policy=zero-or-central-exact zip_entry_max=32 xml_inflate_max_bytes=32768 inflater_state=caller-owned scratch_total_bytes=43272 scratch_lifetime=load-call scratch_wiped=1 local_range_overlap_rejected=1 stored_crc32_verified=1 inflated_crc32_verified=1 compressed_input_exact=1 declared_size_exact=1 descriptor_values_exact=1 art=0 binder=0 bionic=0 general_apk_claim=0 broad_android_compatibility_claim=0 network=disabled emulator_only=1 real_phone_claim=0$" \
  "the ABI-51 APK Envelope-4 profile marker"
stop_qemu
...
wait_for_pattern \
  ...
  "the recovery inherited MultiActionActivity-3 profile marker"
wait_for_pattern \
  "^ANDROIDBOX_APK_ENVELOPE4_PROFILE_OK format=1 abi=${ABI} parent_profile=androidbox-multiaction3 package=${PACKAGE} apk_source=durable-package-store-readback manifest_method=deflate layout_method=deflate classes_dex_method=stored resources_arsc_method=stored data_descriptors=3 incidental_deflated_entries=1 bit3_local_tuple_policy=zero-or-central-exact zip_entry_max=32 xml_inflate_max_bytes=32768 inflater_state=caller-owned scratch_total_bytes=43272 scratch_lifetime=load-call scratch_wiped=1 local_range_overlap_rejected=1 stored_crc32_verified=1 inflated_crc32_verified=1 compressed_input_exact=1 declared_size_exact=1 descriptor_values_exact=1 art=0 binder=0 bionic=0 general_apk_claim=0 broad_android_compatibility_claim=0 network=disabled emulator_only=1 real_phone_claim=0$" \
  "the recovery APK Envelope-4 profile marker"
...
wait_for_pattern "^ANDROID_APP_RESTART_OK .* abi=${ABI} .* crash_request_id=${CRASH_REQUEST_ID} .* errors=0$" \
  "the completed APK Envelope-4 worker replacement"
normalize_log
...
qmp tap 360 688
wait_for_pattern \
  "^ANDROID_APP_RPC_READ_OK sender_image=android-app sender_pid=${NEW_PID} receiver_image=app receiver_pid=${APP_PID} kind=updated request_id=${APPROVE_REQUEST_ID} arg0=${STATUS_ID} " \
...
qmp tap 360 792
wait_for_pattern \
  "^ANDROID_APP_RPC_READ_OK sender_image=android-app sender_pid=${NEW_PID} receiver_image=app receiver_pid=${APP_PID} kind=updated request_id=${REJECT_REQUEST_ID} arg0=${STATUS_ID} " \
...
(
    ...
    relaunch_rpc_messages, replacement_rpc_messages,
) = sys.argv[4:18]
...
if len(install_profiles) != 1:
    raise SystemExit("install boot lacks exactly one inherited MultiActionActivity-3 profile")
install_envelope_profiles = [
    fields(line) for line in install
    if line.startswith("ANDROIDBOX_APK_ENVELOPE4_PROFILE_OK ")
]
if len(install_envelope_profiles) != 1:
    raise SystemExit("install boot lacks exactly one APK Envelope-4 profile")
...
install_envelope_profile_i = next(
    index
    for index, line in enumerate(install)
    if line.startswith("ANDROIDBOX_APK_ENVELOPE4_PROFILE_OK ")
)
...
if not (
    install_envelope_profile_i < install_ready_i
    < install_process_i < install_profile_i
):
...
envelope_profile_i, envelope_profile = exact(
    "ANDROIDBOX_APK_ENVELOPE4_PROFILE_OK "
)
...
if not (
    envelope_profile_i < process_i
    < armed_i < fault_i < reap_i < rebind_i < ready_i < grant_i
    < rebound_i < reopen_i < restart_i < rpc_i < post_relaunch_i
):
    raise SystemExit("APK Envelope-4 restart markers are out of canonical order")
...
require_exact(install_envelope_profiles[0], {
    "format": "1", "abi": abi, "parent_profile": "androidbox-multiaction3",
    "package": "org.bndroid.envelope",
    "apk_source": "durable-package-store-readback",
    "manifest_method": "deflate", "layout_method": "deflate",
    "classes_dex_method": "stored", "resources_arsc_method": "stored",
    "data_descriptors": "3", "incidental_deflated_entries": "1",
    "bit3_local_tuple_policy": "zero-or-central-exact",
    "zip_entry_max": "32", "xml_inflate_max_bytes": "32768",
    "inflater_state": "caller-owned", "scratch_total_bytes": "43272",
    "scratch_lifetime": "load-call",
    "scratch_wiped": "1", "local_range_overlap_rejected": "1",
    "stored_crc32_verified": "1", "inflated_crc32_verified": "1",
    "compressed_input_exact": "1", "declared_size_exact": "1",
    "descriptor_values_exact": "1", "art": "0", "binder": "0",
    "bionic": "0", "general_apk_claim": "0",
    "broad_android_compatibility_claim": "0", "network": "disabled",
    "emulator_only": "1", "real_phone_claim": "0",
}, "install APK Envelope-4 profile")
require_exact(
    envelope_profile,
    install_envelope_profiles[0],
    "recovery APK Envelope-4 profile",
)
require(preview, {
...
if fault_match is None:
    raise SystemExit("controlled APK Envelope-4 data-abort tuple is not exact")
...
require(reopen_marker, {
    ...
    "scene_text_bytes": "45", "text_views": "2", "callback_buttons": "2",
